...
const PLAN_16_OPERATIONS=Object.freeze([
  index('uq_circle_pairing_groups_schedule_owner','circle_pairing_groups',[
    'id','publication_id','scope_key','circle_id','cycle_key','member_count','user_a_id','user_b_id','is_solo',
  ],{unique:true}),
  table('circle_pair_schedules',`CREATE TABLE IF NOT EXISTS circle_pair_schedules (id INTEGER PRIMARY KEY AUTOINCREMENT, schedule_key TEXT NOT NULL UNIQUE CHECK(length(schedule_key)=64 AND schedule_key NOT GLOB '*[^0-9a-f]*'), publication_id INTEGER NOT NULL CHECK(typeof(publication_id)='integer' AND publication_id>0), scope_key TEXT NOT NULL, circle_id INTEGER NOT NULL CHECK(typeof(circle_id)='integer' AND circle_id>0), cycle_key TEXT NOT NULL CHECK(length(cycle_key)=64 AND cycle_key NOT GLOB '*[^0-9a-f]*'), group_id INTEGER NOT NULL CHECK(typeof(group_id)='integer' AND group_id>0), member_count INTEGER NOT NULL DEFAULT 2 CHECK(typeof(member_count)='integer' AND member_count=2), user_a_id INTEGER NOT NULL CHECK(typeof(user_a_id)='integer' AND user_a_id>0), user_b_id INTEGER NOT NULL CHECK(typeof(user_b_id)='integer' AND user_b_id>0 AND user_b_id<>user_a_id), is_solo INTEGER NOT NULL DEFAULT 0 CHECK(typeof(is_solo)='integer' AND is_solo=0), revision INTEGER NOT NULL CHECK(typeof(revision)='integer' AND revision>=1), agreed_time TEXT, created_at TEXT NOT NULL CHECK(length(created_at)=24 AND created_at GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]T[0-9][0-9]:[0-9][0-9]:[0-9][0-9].[0-9][0-9][0-9]Z' AND julianday(created_at) IS NOT NULL AND strftime('%Y-%m-%dT%H:%M:%fZ',created_at)=created_at), updated_at TEXT NOT NULL CHECK(length(updated_at)=24 AND updated_at GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]T[0-9][0-9]:[0-9][0-9]:[0-9][0-9].[0-9][0-9][0-9]Z' AND julianday(updated_at) IS NOT NULL AND strftime('%Y-%m-%dT%H:%M:%fZ',updated_at)=updated_at), UNIQUE(publication_id,group_id), UNIQUE(id,schedule_key,publication_id,scope_key,circle_id,cycle_key,group_id,member_count,user_a_id,user_b_id,is_solo), CHECK(scope_key=('circle:'||circle_id)), CHECK(agreed_time IS NULL OR (length(agreed_time)=24 AND agreed_time GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]T[0-9][0-9]:[0-9][0-9]:[0-9][0-9].[0-9][0-9][0-9]Z' AND julianday(agreed_time) IS NOT NULL AND strftime('%Y-%m-%dT%H:%M:%fZ',agreed_time)=agreed_time)), CHECK(julianday(updated_at)>=julianday(created_at)), FOREIGN KEY(publication_id,scope_key,circle_id,cycle_key) REFERENCES circle_pairing_publications(id,scope_key,circle_id,cycle_key) ON DELETE RESTRICT, FOREIGN KEY(group_id,publication_id,scope_key,circle_id,cycle_key,member_count,user_a_id,user_b_id,is_solo) REFERENCES circle_pairing_groups(id,publication_id,scope_key,circle_id,cycle_key,member_count,user_a_id,user_b_id,is_solo) ON DELETE RESTRICT)`),
  table('circle_pair_schedule_proposals',`CREATE TABLE IF NOT EXISTS circle_pair_schedule_proposals (id INTEGER PRIMARY KEY AUTOINCREMENT, schedule_id INTEGER NOT NULL CHECK(typeof(schedule_id)='integer' AND schedule_id>0), schedule_key TEXT NOT NULL CHECK(length(schedule_key)=64 AND schedule_key NOT GLOB '*[^0-9a-f]*'), publication_id INTEGER NOT NULL CHECK(typeof(publication_id)='integer' AND publication_id>0), scope_key TEXT NOT NULL, circle_id INTEGER NOT NULL CHECK(typeof(circle_id)='integer' AND circle_id>0), cycle_key TEXT NOT NULL CHECK(length(cycle_key)=64 AND cycle_key NOT GLOB '*[^0-9a-f]*'), group_id INTEGER NOT NULL CHECK(typeof(group_id)='integer' AND group_id>0), member_count INTEGER NOT NULL DEFAULT 2 CHECK(typeof(member_count)='integer' AND member_count=2), user_a_id INTEGER NOT NULL CHECK(typeof(user_a_id)='integer' AND user_a_id>0), user_b_id INTEGER NOT NULL CHECK(typeof(user_b_id)='integer' AND user_b_id>0 AND user_b_id<>user_a_id), is_solo INTEGER NOT NULL DEFAULT 0 CHECK(typeof(is_solo)='integer' AND is_solo=0), proposal_key TEXT NOT NULL CHECK(length(proposal_key)=64 AND proposal_key NOT GLOB '*[^0-9a-f]*'), instant TEXT NOT NULL CHECK(length(instant)=24 AND instant GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]T[0-9][0-9]:[0-9][0-9]:[0-9][0-9].[0-9][0-9][0-9]Z' AND julianday(instant) IS NOT NULL AND strftime('%Y-%m-%dT%H:%M:%fZ',instant)=instant), proposed_by INTEGER NOT NULL CHECK(typeof(proposed_by)='integer' AND proposed_by>0 AND proposed_by IN (user_a_id,user_b_id)), created_at TEXT NOT NULL CHECK(length(created_at)=24 AND created_at GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]T[0-9][0-9]:[0-9][0-9]:[0-9][0-9].[0-9][0-9][0-9]Z' AND julianday(created_at) IS NOT NULL AND strftime('%Y-%m-%dT%H:%M:%fZ',created_at)=created_at), UNIQUE(schedule_id,proposal_key), UNIQUE(schedule_id,instant), CHECK(scope_key=('circle:'||circle_id)), FOREIGN KEY(schedule_id,schedule_key,publication_id,scope_key,circle_id,cycle_key,group_id,member_count,user_a_id,user_b_id,is_solo) REFERENCES circle_pair_schedules(id,schedule_key,publication_id,scope_key,circle_id,cycle_key,group_id,member_count,user_a_id,user_b_id,is_solo) ON DELETE RESTRICT, FOREIGN KEY(publication_id,scope_key,circle_id,cycle_key) REFERENCES circle_pairing_publications(id,scope_key,circle_id,cycle_key) ON DELETE RESTRICT, FOREIGN KEY(group_id,publication_id,scope_key,circle_id,cycle_key,member_count,user_a_id,user_b_id,is_solo) REFERENCES circle_pairing_groups(id,publication_id,scope_key,circle_id,cycle_key,member_count,user_a_id,user_b_id,is_solo) ON DELETE RESTRICT, FOREIGN KEY(proposed_by) REFERENCES auth_accounts(id) ON DELETE RESTRICT)`),
