...
- `MULTI_CIRCLE_CONTROL_PLANE_ENABLED=true` to enable session-bound circle selection and circle-scoped roster/invitation management. Keep the independent `MULTI_CIRCLE_AVAILABILITY_ENABLED=false` until selected-circle availability has been rehearsed; enabling it scopes only the dated availability endpoint. Pairing, history, schedule, chat, execution, workspace, video, and AI routes still return `409 circle_feature_unavailable` for multi-circle accounts until their records are tenant-owned; see `docs/ACTIVE_CIRCLE_CONTEXT.md`.
