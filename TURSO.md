...
4. Complete and verify the production backup/restore work tracked by issue #38 and the migration controls tracked by issue #43 before changing production data.
   ...
   For this release, apply every pending migration in v13, v14, then v15 order
   ...
   promoting the v15-aware runtime or enabling a credential consumer. The
...
- Secondary coordination — after the complete managed ledger through v15 is ready, the default-off `SECONDARY_CIRCLE_COORDINATION_ENABLED` flag lets a selected secondary owner publish a circle-owned current-cycle result and lets active members view it. Migration v13 owns the coordination tables, migration v14 owns circle creation, and migration v15 completes exact runtime readiness for this release. The separately default-off `SECONDARY_CIRCLE_PAIRING_EMAIL_ENABLED` flag queues dashboard-only result mail and is effective only with the full control-plane, availability, and coordination chain. Keep it false until a separate provider canary succeeds. Responses and messages never contain legacy room capabilities and do not authorize schedule, chat, video, execution, recap, or AI. See [the selected-circle pairing runbook](docs/SELECTED_CIRCLE_PAIRING.md).
- Circle creation — migration v14 owns the creation receipt, but this release exposes same-origin `POST /api/circles` only after the complete managed ledger through v15 is ready and all four credential controls are adopted under the central rollout. Creation writes only a secondary circle, owner membership, one creation audit, one durable idempotency receipt, and the initiating session's selected context. See [the circle creation runbook](docs/CIRCLE_CREATION.md).
...
- `GET|PUT|POST /api/circles` — lists session-visible circles, selects one by public ID plus expected generation, or atomically creates/selects a secondary circle from an exact name plus opaque request ID. Migration v14 owns POST storage; this release additionally requires exact readiness through v15 and completed credential-control adoption before runtime promotion. POST enforces 10 active owned circles and the 100-membership read ceiling, and returns no internal IDs.
