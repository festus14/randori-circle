...
- `db/schema-manifest.js` is the current contract: 44 application tables and 45 named indexes.
...
# v3 through v11 pending before using its new full-set fingerprint for the upgrade.
npm run --silent db:migrate -- \
  status --database file:///absolute/path/to/restored-randori.db
