...
test('status uses a read-only token and returns one actionable pending migration',async()=>{
  const item=fixture();
  try{
    await installManaged(item.path,12);
    ...
    assert.deepEqual(result.pendingVersions,[13]);
...
test('apply advances one managed version per run and a fully migrated run is an explicit no-op',async()=>{
  ...
  try{
    ...
    assert.deepEqual(first.appliedVersions,[13]);
    assert.equal(first.fromVersion,12);
    assert.equal(first.toVersion,13);
    assert.equal((await state(item.path)).currentVersion,13);
    assert.deepEqual(platform.calls.filter(call=>call[0]==='token'),[
    ...
    assert.equal(second.fromVersion,13);
    assert.equal(second.toVersion,13);
  }finally{ item.close(); }
});
