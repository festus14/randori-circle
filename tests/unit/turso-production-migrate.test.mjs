...
test('status uses a read-only token and returns one actionable pending migration',async()=>{
  const item=fixture();
  try{
    await installManaged(item.path,6);
    ...
    assert.deepEqual(result.pendingVersions,[7]);
...
test('apply advances one managed version per run and a fully migrated run is an explicit no-op',async()=>{
  ...
  try{
    ...
    assert.deepEqual(first.appliedVersions,[7]);
    ...
    assert.equal(second.fromVersion,7);
    assert.equal(second.toVersion,7);
  }finally{ item.close(); }
});
