...
test('status uses a read-only token and returns one actionable pending migration',async()=>{
  const item=fixture();
  try{
    await installManaged(item.path,8);
    ...
    assert.deepEqual(result.pendingVersions,[9]);
...
test('apply advances one managed version per run and a fully migrated run is an explicit no-op',async()=>{
  ...
  try{
    ...
    assert.deepEqual(first.appliedVersions,[9]);
    assert.equal(first.fromVersion,8);
    assert.equal(first.toVersion,9);
    ...
    assert.equal(second.fromVersion,9);
    assert.equal(second.toVersion,9);
  }finally{ item.close(); }
});
