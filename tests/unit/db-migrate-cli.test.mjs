...
test('status projection exposes the mutation fingerprint without paths or schema details',()=>{
  ...
  assert.deepEqual(blocked.pendingVersions,[2,3,4,5,6,7]);
...
test('apply forwards the full migration set and reports only migration versions',async()=>{
  ...
  const execution=await main({
    ...
    apply:async(_client,options)=>{
      ...
      return {
        ok:true,mode:'apply',fromVersion:1,toVersion:7,latestVersion:7,
        applied:[{version:7,name:'private-name',checksum:'secret-checksum',executionMs:4}],
  ...
  assert.deepEqual(calls,[
    'prepare',
    {expectedStateFingerprint:FINGERPRINT,migrationVersions:[1,2,3,4,5,6,7]},
    'close',
  ]);
  assert.equal(execution.result.result,'applied');
  assert.equal(execution.result.throughVersion,7);
  assert.equal(execution.result.latestVersion,7);
  assert.deepEqual(execution.result.target,{kind:'local-file',existedBefore:true});
  assert.deepEqual(execution.result.appliedVersions,[7]);
...
test('an exact unmanaged v2 rehearsal can adopt its prefix and then apply pending migrations',async()=>{
  ...
  try{
    ...
    assert.equal(latestInspection.result.throughVersion,7);
    assert.equal(latestInspection.result.latestVersion,7);
    ...
    assert.equal(prefixInspection.result.latestVersion,7);
    ...
    assert.equal(adoption.result.latestVersion,7);
    ...
    assert.deepEqual(managedV2.result.ledger,{present:true,currentVersion:2,latestVersion:7});
    assert.deepEqual(managedV2.result.pendingVersions,[3,4,5,6,7]);
    ...
    assert.deepEqual(upgraded.result.appliedVersions,[3,4,5,6,7]);
    assert.equal(upgraded.result.toVersion,7);
    ...
    assert.equal(noop.result.fromVersion,7);
    assert.equal(noop.result.toVersion,7);
    assert.equal(noop.result.throughVersion,7);
    assert.equal(noop.result.latestVersion,7);
...
test('prefix adoption refuses mismatched fingerprints and drift without writing a ledger',async()=>{
  ...
  try{
    ...
    assert.equal(stale.result.latestVersion,7);
