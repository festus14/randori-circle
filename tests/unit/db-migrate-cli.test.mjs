...
test('status projection exposes the mutation fingerprint without paths or schema details',()=>{
  ...
  assert.deepEqual(blocked.pendingVersions,[2,3,4,5]);
...
test('apply forwards the full migration set and reports only migration versions',async()=>{
  ...
  const execution=await main({
    ...
    apply:async(_client,options)=>{
      ...
      return {
        ok:true,mode:'apply',fromVersion:1,toVersion:5,latestVersion:5,
        applied:[{version:5,name:'private-name',checksum:'secret-checksum',executionMs:4}],
  ...
  assert.deepEqual(calls,[
    'prepare',
    {expectedStateFingerprint:FINGERPRINT,migrationVersions:[1,2,3,4,5]},
    'close',
  ]);
  assert.equal(execution.result.result,'applied');
  assert.equal(execution.result.throughVersion,5);
  assert.equal(execution.result.latestVersion,5);
  assert.deepEqual(execution.result.target,{kind:'local-file',existedBefore:true});
  assert.deepEqual(execution.result.appliedVersions,[5]);
...
test('an exact unmanaged v2 rehearsal can adopt its prefix and then apply pending migrations',async()=>{
  ...
  try{
    ...
    assert.equal(latestInspection.result.throughVersion,5);
    assert.equal(latestInspection.result.latestVersion,5);
    ...
    assert.equal(prefixInspection.result.latestVersion,5);
    ...
    assert.equal(adoption.result.latestVersion,5);
    ...
    assert.deepEqual(managedV2.result.ledger,{present:true,currentVersion:2,latestVersion:5});
    assert.deepEqual(managedV2.result.pendingVersions,[3,4,5]);
    ...
    assert.deepEqual(upgraded.result.appliedVersions,[3,4,5]);
    assert.equal(upgraded.result.toVersion,5);
    ...
    assert.equal(noop.result.fromVersion,5);
    assert.equal(noop.result.toVersion,5);
    assert.equal(noop.result.throughVersion,5);
    assert.equal(noop.result.latestVersion,5);
...
test('prefix adoption refuses mismatched fingerprints and drift without writing a ledger',async()=>{
  ...
  try{
    ...
    assert.equal(stale.result.latestVersion,5);
