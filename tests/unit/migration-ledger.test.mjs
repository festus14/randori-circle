...
test('executable migrations are contiguous and fingerprint every executable operation',()=>{
  ...
  assert.equal(LATEST_MIGRATION_VERSION,18);
...
test('ledger rejects gaps, future versions, checksum drift, and owned schema additions',async()=>{
  ...
  try{
    ...
    assert.throws(()=>validateMigrationLedger([row(1),row(2),row(3),row(4),row(5),row(6),row(7),row(8),row(9),row(10),row(11),row(12),row(13),row(14),row(15),row(16),row(17),row(18),row(19)],EXECUTABLE_MIGRATIONS),/newer/i);
