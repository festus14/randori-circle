...
test('fresh apply is transactional, seeds an open rollout, and repeats as a no-op',async()=>{
  ...
  try{
    ...
    assert.deepEqual(result.applied.map(item=>item.version),[1,2,3,4,5,6,7,8]);
    ...
    assert.deepEqual(ledger.rows.map(row=>[Number(row.version),row.disposition]),[[1,'applied'],[2,'applied'],[3,'applied'],[4,'applied'],[5,'applied'],[6,'applied'],[7,'applied'],[8,'applied']]);
    ...
    assert.equal(repeat.fromVersion,8);
    assert.equal(repeat.toVersion,8);
...
test('a valid managed v1 database resumes through only the pending migrations',async()=>{
  ...
  try{
    ...
    assert.equal(resumed.toVersion,8);
    assert.deepEqual(resumed.applied.map(item=>item.version),[2,3,4,5,6,7,8]);
    const ledger=await fixture.db.execute('SELECT version FROM schema_migrations ORDER BY version');
    assert.deepEqual(ledger.rows.map(row=>Number(row.version)),[1,2,3,4,5,6,7,8]);
  }finally{ fixture.close(); }
});
...
test('exact open schema can be explicitly adopted and adoption is auditable',async()=>{
  ...
  try{
    ...
    assert.equal(adopted.toVersion,8);
    ...
    assert.deepEqual(rows.rows.map(row=>[Number(row.version),Number(row.execution_ms),row.disposition]),[
      [1,0,'adopted'],[2,0,'adopted'],[3,0,'adopted'],[4,0,'adopted'],[5,0,'adopted'],[6,0,'adopted'],[7,0,'adopted'],[8,0,'adopted'],
    ]);
...
test('concurrent callers cannot silently apply from the same stale fingerprint',async()=>{
  ...
  try{
    ...
    assert.equal(final.currentVersion,8);
