...
test('current schema passes read-only inspection and tolerates the retired AI table',async()=>{
  ...
  assert.deepEqual(status.summary,{
    expectedTables:55,presentTables:55,expectedIndexes:63,presentIndexes:63,
    expectedTriggers:17,presentTriggers:17,blockers:0,warnings:0,
  });
...
test('empty database produces a non-executable, checksum-bearing plan',async()=>{
  ...
  assert.equal(status.blockers.length,135);
  assert.equal(plan.readOnly,true);
  assert.equal(plan.executable,false);
  assert.equal(plan.actions.length,135);
