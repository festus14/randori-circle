import { getClient, getJwtSecret, getCronSecret, getAdminEmails, isoWeekLabel, shuffleArray } from './_db.js';
...
async function handleReshuffle(req,res){
  ...
  const auth = req.headers.authorization||'';
  const m = auth.match(/^Bearer\s+(.+)$/);
  if (!m) return res.status(401).json({ error:'missing Bearer <redacted> — sign in as admin' });
  ...
  if (!callerIsAdmin){ const envAdmins=Array.from(getAdminEmails()); return res.status(403).json({ error:'forbidden: admin only', required_admins:envAdmins, you_are:callerEmail||payload.email||'unknown', hint:'Set ADMIN_EMAILS env var comma-separated to include this email, or have existing admin promote you' }); }
  ...
  let authRs; try{ authRs=await db.execute(`SELECT id, display_name as name, email, color, is_available FROM auth_accounts WHERE COALESCE(is_available,1)=1 ORDER BY id`);}catch{ authRs=await db.execute(`SELECT id, display_name as name, email, color FROM auth_accounts ORDER BY id`); }
  ...
  if (existing.rows.length){ weekId=existing.rows[0].id; await db.execute({ sql:`DELETE FROM pairing_groups WHERE week_id=?`, args:[weekId]}); await db.execute({ sql:`UPDATE pairing_weeks SET week_start=? WHERE id=?`, args:[now.toISOString(), weekId]}); } else { const ins=await db.execute({ sql:`INSERT INTO pairing_weeks (week_label, week_start, focus) VALUES (?,?,?) RETURNING id`, args:[weekLabel, now.toISOString(),'both']}); weekId=ins.rows[0].id; }
...
async function handleWeekly(req,res){
  ...
  if (authRs.rows.length){
    allAccounts=authRs.rows.map(r=>({ id:r.id, name:r.name, color:r.color, email:r.email, is_available:r.is_available===null||r.is_available===undefined?1:(r.is_available?1:0), source:'auth'}));
...
export default async function handler(req,res){
  ...
  // also allow direct?secret= fallback for weekly
  ...
  return res.status(404).json({ error:`unknown ops endpoint '${ep}'`, available:['availability','reshuffle','weekly','promote via reshuffle?action=promote'] });
}
